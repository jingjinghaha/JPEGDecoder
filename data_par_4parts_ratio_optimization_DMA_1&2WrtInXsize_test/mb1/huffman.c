...
void unpack_block( FBlock * T, int select)
{
	...
	symbol = get_symbol( HUFF_ID(DC_CLASS, comp[select].DC_HT));//read data in this function
	value = reformat(get_bits( symbol), symbol);

	value += comp[select].PRED;
	comp[select].PRED = value; //VLD
	T->linear[0] = value * QTable[(int)comp[select].QT]->linear[0];//DC dequantify

	/* Now get all 63 AC values: */
	for (i = 1; i < 64; i++) {    //AC dequantify
