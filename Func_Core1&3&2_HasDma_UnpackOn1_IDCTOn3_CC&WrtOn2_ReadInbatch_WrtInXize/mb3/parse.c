...
{				/* skip a segment we don't want */
	...
	// mk_mon_debug_info(158471);
	// mk_mon_debug_info(size-2);
	// mk_mon_debug_info(158471);
...
//mk_mon_debug_info("Enter into process_MCU\n");
	...
	for (curcomp = 0; MCU_valid[curcomp] != -1; curcomp++) {
		cheap_read_fifo(consumer13, producer13, FBuff, BLOCK_IF_FULL);
		//unpack_block( FBuff, MCU_valid[curcomp]);	/* pass index to HT,QT,pred */ //ZZ
		IDCT(FBuff, MCU_buff[curcomp]);  ///IDCT
		cheap_write_fifo(producer32, consumer32, MCU_buff[curcomp], BLOCK_IF_FULL);
	}
	MCU_column++;
	return 1;
