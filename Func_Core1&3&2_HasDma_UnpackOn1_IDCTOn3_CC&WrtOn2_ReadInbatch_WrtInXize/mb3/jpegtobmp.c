...
volatile fifo_t* producer13 = (fifo_t*) mb1_cmemin0_pt_REMOTEADDR;
volatile fifo_t* consumer13 = (fifo_t*) mb3_cmemin0_BASEADDR;
volatile fifo_t* producer32 = (fifo_t*) (mb3_cmemin0_BASEADDR + 1 * sizeof(fifo_t) + FIFO_SIZE1_3 * sizeof(FBlock) + 4);
volatile fifo_t* consumer32 = (fifo_t*) mb2_cmemin0_pt_REMOTEADDR;
volatile int* producer_data32 = (int*) (mb2_cmemin0_pt_REMOTEADDR + 1 * sizeof(fifo_t));
volatile int* consumer_data32 = (int*) (mb2_cmemin0_BASEADDR + 1 * sizeof(fifo_t));
volatile int* fifo_sync13 = (int*) (mb3_cmemin0_BASEADDR + 1 * sizeof(fifo_t) + FIFO_SIZE1_3 * sizeof(FBlock));
volatile int* fifo_sync32 = (int*) (mb2_cmemin0_pt_REMOTEADDR + 1 * sizeof(fifo_t) + FIFO_SIZE3_2 * sizeof(PBlock));
...
int main (void)
{
	...
	cheap_init_fifo(
			...
			FIFO_SIZE3_2,  // fifo size
			(unsigned int)producer_data32,
			(unsigned int)consumer_data32,
#ifdef USE_DMA
			mb3_dma0_BASEADDR,
			mb3_cmemout0_BASEADDR+sizeof(int),
			mb2_dma0_BASEADDR,
	...
	while (*fifo_sync13 == 0) {}
#ifdef USE_DMA
	unsigned int *f = (unsigned int *)mb3_cmemout0_BASEADDR;	
	*f =1 ;
	hw_dma_send_addr((void*)fifo_sync32, (void*)f, 1, (void*)mb3_dma0_BASEADDR);
	
#else
	// signal to core 2 that the FIFO is initialized and can be read from.
	*fifo_sync32 = 1;
#endif
	//mk_mon_debug_info(22220);
	//mk_mon_debug_info(1111);
