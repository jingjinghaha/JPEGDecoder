...
void color_conversion(int goodrows, int goodcolumns)
{
	...
	unsigned int x_off = MCU_column*MCU_sx;

 if (n_comp > 1)
{	for (i = 0; i < goodrows; i++) {	/* pixel rows */
		...
		for (j = 0; j < goodcolumns; j++) {	/* pixel columns */
			...
			y = MCU_buff[inv_ndx_0 + (jp_0 >> 3)]->block[ip_0_lsbs][jp_0 & 7];
				...
				b = y + ((227 * rcb) >> 7);
				ColorBuffer[(i * x_size) + j + x_off] = ( (Saturate(r) << 16) | (Saturate(g) << 8) | Saturate(b));
			...
			for (i = 0; i < goodrows; i++)
				...
				{
					*(((char*)(ColorBuffer+i * x_size + j + x_off))+0) = (MCU_buff[0])->linear[(i * MCU_sx + j)];
					*(((char*)(ColorBuffer+i * x_size + j + x_off))+1) = (MCU_buff[0])->linear[(i * MCU_sx + j)];
					*(((char*)(ColorBuffer+i * x_size + j + x_off))+2) = (MCU_buff[0])-> linear[(i * MCU_sx + j)];
				}
				//ColorBuffer[(i * x_size) + j + x_off][0] = (  (MCU_buff[0][(i * MCU_sx + j)]) << 16) | ((MCU_buff[0][(i * MCU_sx + j)]) << 8) | (MCU_buff[0][(i * MCU_sx + j)]));
}
			//ColorBuffer[(i * MCU_sx) + j] = ((0xFF << 24) | (Saturate(y) << 16) | (Saturate(y) << 8) | Saturate(y));
			//ColorBuffer[(y_off) + (j+x_off)] = ((0xFF << 24) | (Saturate(r) << 16) | (Saturate(g) << 8) | Saturate(b));
			/*
	...
	// for (i = 0; i < MCU_sy; i++) {	/* pixel rows */
		...
		// for (j = 0; j < MCU_sx; j++) {	/* pixel columns */
			...
			// if (n_comp > 1) {
				...
				// g = y - ((11 * rcb) >> 5) - ((183 * rcr) >> 8);
				// b = y + ((227 * rcb) >> 7);
				// ColorBuffer[(i * x_size) + j + x_off] = ((0xFF << 24) | (Saturate(r) << 16) | (Saturate(g) << 8) | Saturate(b));
			// }
			// else {
				// ColorBuffer[(i * x_size) + j + x_off] = ((0xFF << 24) | (Saturate(y) << 16) | (Saturate(y) << 8) | Saturate(y));
			// }
			// //ColorBuffer[(i * MCU_sx) + j] = ((0xFF << 24) | (Saturate(y) << 16) | (Saturate(y) << 8) | Saturate(y));
			// //ColorBuffer[(y_off) + (j+x_off)] = ((0xFF << 24) | (Saturate(r) << 16) | (Saturate(g) << 8) | Saturate(b));
			// /*
