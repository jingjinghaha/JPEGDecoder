...
//mk_mon_debug_info("Enter into process_MCU\n");
	...
	long x_offset=0;
	unsigned int *cmem_out_buffer = (unsigned int *)(consumer32->dma_data_address+sizeof(int)+DMA_BUF_BYTESIZE);
	...
	cheap_read_fifo(consumer32, producer32, MCU_buff[curcomp], BLOCK_IF_FULL);
	}
	...
	if( (x_offset >= x_size*MCU_sy) )
	{
		for (i = 0; i < goodrows; i++) {
			...
			hw_dma_send_addr( 	(unsigned int*) (shared_pt_REMOTEADDR+(MCU_BLOCK*1024*4)), //Baseaddr + (amount of MCU rows * bits in MCU y direction * 1024 bits in x dir * 4 bytes)
								cmem_out_buffer,
								x_size,//1024
