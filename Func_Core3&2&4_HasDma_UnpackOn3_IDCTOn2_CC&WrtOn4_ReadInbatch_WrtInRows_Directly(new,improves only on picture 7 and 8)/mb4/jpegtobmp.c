...
volatile fifo_t* producer24 = (fifo_t*) (mb2_cmemin0_pt_REMOTEADDR + 1 * sizeof(fifo_t) + FIFO_SIZE3_2 * sizeof(FBlock) + 4);
volatile fifo_t* consumer24 = (fifo_t*) mb4_cmemin0_BASEADDR;
volatile int* fifo_sync24 = (int*) (mb4_cmemin0_BASEADDR + 1 * sizeof(fifo_t) + FIFO_SIZE2_4 * sizeof(PBlock));
...
unsigned char *ColorBuffer = (unsigned char *)(mb4_cmemout0_BASEADDR+sizeof(int)+DMA_BUF_BYTESIZE);;
...
int JpegToBmp()
{
	...
	do {
		...
		case SOF_MK:  //ffc0 start of the frame
			...
			//ColorBuffer = (unsigned char *)mk_malloc((size_t) MCU_sx * MCU_sy * 4);
			FBuff = (FBlock *) mk_malloc(sizeof(FBlock));
			PBuff = (PBlock *) mk_malloc(sizeof(PBlock));

			if (   (FBuff == NULL) || (PBuff == NULL)) {
