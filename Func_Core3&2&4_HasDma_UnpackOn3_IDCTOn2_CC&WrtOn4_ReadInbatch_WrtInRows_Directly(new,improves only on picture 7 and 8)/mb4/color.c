...
else
{
	...
	{	int i_times_MCU_sx = i * MCU_sx;
				...
				{	
					offset = 4 * (i_times_MCU_sx + j);
					ColorBuffer[offset + 2] = (MCU_buff[0])->linear[(i * MCU_sx + j)];
					ColorBuffer[offset + 1] = (MCU_buff[0])->linear[(i * MCU_sx + j)];
					ColorBuffer[offset + 0] = (MCU_buff[0])-> linear[(i * MCU_sx + j)];
				}
	}
}
}