...
//mk_mon_debug_info("Enter into process_MCU\n");
	...
	for (curcomp = 0; MCU_valid[curcomp] != -1; curcomp++) {
		cheap_read_fifo(consumer24, producer24, MCU_buff[curcomp], BLOCK_IF_FULL);
	}
...
#else
//unsigned int *d = (unsigned int *)(consumer24->dma_data_address+sizeof(int)+DMA_BUF_BYTESIZE);
if (n_comp > 1)
{
	for (i = 0; i < goodrows; i++)
	{	
		...
		hw_dma_send_addr((void *)(bmp_buffer + offset + (i << 12)), (void *)ColorBuffer+i*MCU_sx*4, goodcolumns,(void *)consumer24->dma_address);			
	}
