...
unsigned int FGETC()
{
	...
	unsigned int *d = (unsigned int *)(producer24->dma_data_address+sizeof(FBlock) );
	unsigned int c;
	unsigned int tmp;
	...
	if ( tmp== 0)
	{
		...
		while(hw_dma_status_addr((void *)producer24->dma_address));
		hw_dma_receive_addr((void *)d, (void *)(jpg_from_host+vld_count / 4), DMA_BUF_WORDSIZE, (void *)producer24->dma_address);
	...
	vld_count++;
	return c;
	//unsigned int *d = (unsigned int *)(consumer32->dma_data_address+sizeof(FBlock));
	// unsigned int *d = (unsigned int *)(consumer32->dma_data_address);
	// unsigned int c;
	// while(hw_dma_status_addr((void *)consumer32->dma_address));
	// hw_dma_receive_addr((void *)d, (void *)(jpg_from_host+vld_count / 4), 1, (void *)consumer32->dma_address);
