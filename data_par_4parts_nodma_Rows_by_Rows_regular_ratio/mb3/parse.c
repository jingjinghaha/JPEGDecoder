...
if (MCU_row < (3*my_size/4) && MCU_row >= my_size/2)	
 ...
 */
	...
	for (i=0; i < goodrows;i++)
	 ...
	 for(kk=0; kk< goodcolumns;kk++)
		...
		bmp_buffer[(((row_buffer<<10) + col_buffer)<<2)+0] = ColorBuffer[3*index_mcu+0];
		bmp_buffer[(((row_buffer<<10) + col_buffer)<<2)+0] = ColorBuffer[3*index_mcu+1];
		bmp_buffer[(((row_buffer<<10) + col_buffer)<<2)+0] = ColorBuffer[3*index_mcu+2];
		}
	 }
	else
	for (i=0; i < goodrows;i++)
	 ...
	 for(kk=0; kk< goodcolumns;kk++)
		...
		col_buffer = MCU_column*MCU_sx+kk;
		index_mcu = i * MCU_sx + kk;
		bmp_buffer[(((row_buffer<<10) + col_buffer)<<2)+0] = ColorBuffer[index_mcu];
		bmp_buffer[(((row_buffer<<10) + col_buffer)<<2)+1] = ColorBuffer[index_mcu];
		bmp_buffer[(((row_buffer<<10) + col_buffer)<<2)+2] = ColorBuffer[index_mcu];
		}
	 }*/
	for (i = 0; i < goodrows; i++)   
		for (kk = 0; kk < goodcolumns; kk++)
	...
	if(n_comp==1){
		for (i = 0; i < goodrows; i++)   
			for (kk = 0; kk < goodcolumns; kk++){
			// for (k = 1; k < 3; k++)
				index_mcu = n_comp*(i * MCU_sx + kk);
				memmove(bmp_buffer + ((offset + (i<<10) + kk)<<2), ColorBuffer + index_mcu, n_comp);
				memmove(bmp_buffer + ((offset + (i<<10) + kk)<<2)+1, ColorBuffer + index_mcu, n_comp);
				memmove(bmp_buffer + ((offset + (i<<10) + kk)<<2)+2, ColorBuffer + index_mcu, n_comp);
			}
	}
}
else if(MCU_row < my_size/2)
for (curcomp = 0; MCU_valid[curcomp] != -1; curcomp++) {
