...
int process_MCU()
{
	...
	if (MCU_row>=(my_size/2))
	{
		for (curcomp = 0; MCU_valid[curcomp] != -1; curcomp++) {
			cheap_read_fifo(consumer42, producer42, FBuff, BLOCK_IF_FULL);
		...
		if( (x_offset >= x_size*MCU_sy) )
		{
			for (i = 0; i < MCU_sy; i++) {
				...
				DMA_usage( 	(void *) (shared_pt_REMOTEADDR+((my_size/2)*MCU_sy*1024*4)+(MCU_BLOCK*1024*4)), 
									(void *) cmem_out_buffer,
									x_size,//1024
									(void *) (mb1_dma0_BASEADDR)
								);
				hw_dma_send_addr( 	(void *) (shared_pt_REMOTEADDR+((my_size/2)*MCU_sy*1024*4)+(MCU_BLOCK*1024*4)), 
									(void *) cmem_out_buffer,
									x_size,//1024
									(void *) (mb1_dma0_BASEADDR)
								);
