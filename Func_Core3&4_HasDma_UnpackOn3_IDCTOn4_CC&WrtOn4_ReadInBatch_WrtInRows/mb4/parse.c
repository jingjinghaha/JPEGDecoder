...
//mk_mon_debug_info("Enter into process_MCU\n");
	...
	for (curcomp = 0; MCU_valid[curcomp] != -1; curcomp++) {
		cheap_read_fifo(consumer34, producer34, FBuff, BLOCK_IF_FULL);
		//unpack_block( FBuff, MCU_valid[curcomp]);	/* pass index to HT,QT,pred */ //ZZ
		IDCT(FBuff, MCU_buff[curcomp]);  ///IDCT
	}
	...
	offset = (MCU_row * MCU_sy * 1024 + MCU_column * MCU_sx)<<2;
#ifndef USE_DMA
	if (n_comp > 1)
	for (i = 0; i < goodrows; i++)
		for (kk = 0; kk < goodcolumns; kk++)
			memmove(bmp_buffer + offset +  ( ((i << 10) + kk)<<2), ColorBuffer + n_comp *(i * MCU_sx + kk), n_comp );
	else
		for (i = 0; i < goodrows; i++)
			for (kk = 0; kk < goodcolumns;kk++)
			{
				memmove(bmp_buffer + offset +  ( ((i <<10) + kk)<<2) +0, ColorBuffer + (i * MCU_sx + kk),1);
				memmove(bmp_buffer + offset +  ( ((i <<10) + kk)<<2) +1, ColorBuffer + (i * MCU_sx + kk),1);
				memmove(bmp_buffer + offset +  ( ((i <<10) + kk)<<2) +2, ColorBuffer + (i * MCU_sx + kk),1);
			}  
#else
unsigned int *d = (unsigned int *)(consumer34->dma_data_address+sizeof(int)+DMA_BUF_BYTESIZE);
if (n_comp > 1)
{
	for (i = 0; i < goodrows; i++)
	{	
		...
		DMA_usage((void *)(bmp_buffer + offset + (i << 12)), (void *)d, goodcolumns,(void *)consumer34->dma_address);
		hw_dma_send_addr((void *)(bmp_buffer + offset + (i << 12)), (void *)d, goodcolumns,(void *)consumer34->dma_address);			
	}
...
/* 	if (n_comp > 1)
	for (i=0; i < goodrows;i++)
     {
	 row_buffer = MCU_row * MCU_sy + i;
	 for(kk=0; kk< goodcolumns;kk++)
		{	
		col_buffer = MCU_column*MCU_sx+kk;		
		index_mcu = i * MCU_sx + kk;
		bmp_buffer[4*(row_buffer*1024 + col_buffer)+0] = ColorBuffer[3*index_mcu+0];
		bmp_buffer[4*(row_buffer*1024 + col_buffer)+1] = ColorBuffer[3*index_mcu+1];
		bmp_buffer[4*(row_buffer*1024 + col_buffer)+2] = ColorBuffer[3*index_mcu+2];
		}
	 }
	else
	for (i=0; i < goodrows;i++)
	 ...
	 for(kk=0; kk< goodcolumns;kk++)
		...
		col_buffer = MCU_column*MCU_sx+kk;
		index_mcu = i * MCU_sx + kk;
		bmp_buffer[4*(row_buffer*1024 + col_buffer)+0] = ColorBuffer[index_mcu];
		bmp_buffer[4*(row_buffer*1024 + col_buffer)+1] = ColorBuffer[index_mcu];
		bmp_buffer[4*(row_buffer*1024 + col_buffer)+2] = ColorBuffer[index_mcu];
		}
	 } */
