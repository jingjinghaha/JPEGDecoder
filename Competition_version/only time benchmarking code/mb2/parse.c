	...
	{	
		...
		if( (x_offset >= x_size*MCU_sy) )
		{
			for (i = 0; i < MCU_sy; i++) {
				...
				// DMA_usage( 	(void *) (shared_pt_REMOTEADDR+(parts[1]*MCU_sy*1024*4)+(MCU_BLOCK*1024*4)), 
				// 					(void *) cmem_out_buffer,
				// 					x_size,//1024
				// 					(void *) (mb2_dma0_BASEADDR)
				// 				);
				hw_dma_send_non_block_addr( 	(void *) (shared_pt_REMOTEADDR+(parts[1]*MCU_sy*1024*4)+(MCU_BLOCK*1024*4)), 
									(void *) cmem_out_buffer,
									x_size,//1024
									(void *) (mb2_dma0_BASEADDR)
								);
