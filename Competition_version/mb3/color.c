...
if (n_comp > 1) {
	for (i = 0; i < goodrows; i++) {	/* pixel rows */
		...
		for (j = 0; j < goodcolumns; j++) {	/* pixel columns */
			...
			ColorBuffer[(i * MCU_sx) + j] = ((0xFF << 24) | (Saturate(r) << 16) | (Saturate(g) << 8) | Saturate(b));
		}
	}
}
else{
		for (i = 0; i < goodrows; i++)
			...
			{
				*(((char*)(ColorBuffer+i * MCU_sx + j))+0) = (MCU_buff[0])->linear[(i * MCU_sx + j)];
				*(((char*)(ColorBuffer+i * MCU_sx + j))+1) = (MCU_buff[0])->linear[(i * MCU_sx + j)];
				*(((char*)(ColorBuffer+i * MCU_sx + j))+2) = (MCU_buff[0])->linear[(i * MCU_sx + j)];
			}
}
}
