...
unsigned int FGETC()
{
	...
	unsigned int *d = (unsigned int *)(consumer24->dma_data_address+sizeof(int) );
	unsigned int c;
	unsigned int tmp;
	...
	if ( tmp == 0)
	{
		...
		hw_dma_receive_addr((void *)d, (void *)(jpg_from_host+vld_count / 4), DMA_BUF_WORDSIZE, (void *)consumer24->dma_address);
