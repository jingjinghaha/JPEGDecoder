...
int WordSize =  mb1_cmemout0_SIZE >> 3;
int ByteSize =  mb1_cmemout0_SIZE >> 1;
...
int process_MCU()
{
	...
	if ((MCU_row < parts[1]) && (MCU_row >= parts[0]) )
	{
		...
		if( (x_offset >= x_size*MCU_sy) ) 
		{
			...
			//y_offset = i+ (mcu_count*MCU_sy
				mb1_offset = (( i + (MCU_row*MCU_sy)) *4*1024);
				for (j = 0; j < x_size; j++) {
					cmem_out_buffer[j] = ColorBuffer[j + i*x_size ]; //0XFF000000;
				}
					hw_dma_send_non_block_addr( 	(unsigned int*) (shared_pt_REMOTEADDR+mb1_offset), //Baseaddr + (amount of MCU rows * bits in MCU y direction * 1024 bits in x dir * 4 bytes)
														cmem_out_buffer,
														x_size,
														(unsigned int*) (mb1_dma0_BASEADDR)
													);
			}
			mcu_count = 0;
		}
