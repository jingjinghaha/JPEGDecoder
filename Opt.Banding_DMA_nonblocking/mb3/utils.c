...
void free_structures()
{
	...
	for (i = 0; i < 4; i++) {
		if (QTvalid[i])	mk_free(QTable[i]);
	}

	if (ColorBuffer != NULL)	mk_free(ColorBuffer);
	if (FrameBuffer != NULL)	mk_free(FrameBuffer);
	if (PBuff != NULL)			mk_free(PBuff);
	if (FBuff != NULL)			mk_free(FBuff);
