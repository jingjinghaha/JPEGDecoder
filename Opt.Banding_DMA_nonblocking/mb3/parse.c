...
extern unsigned int* cmem_out_buffer0;
extern unsigned int* cmem_out_buffer1;
// int WordSize =  mb3_cmemout0_SIZE >> 3;
// int ByteSize =  mb3_cmemout0_SIZE >> 1;
int WordSize =  mb3_cmemout0_SIZE >> 2;
int ByteSize =  mb3_cmemout0_SIZE;
...
int process_MCU()
{
	...
	if ((MCU_row < parts[3]) && (MCU_row >= parts[2]))
	{		
		...
		//mb3_offset = ((3*my_size/4)*mx_size*MCU_sx*MCU_sy*4);	
		...
		for (i = 0; i < goodrows; i++) 
		{
			...
			//{
				for (j = 0; j < goodcolumns; j++) {
					 cmem_out_buffer0[j] = (ColorBuffer[(i * MCU_sx) + j]);
				}
				hw_dma_send_non_block_addr( 	(unsigned int*) (shared_pt_REMOTEADDR+((col_count*4)+(i*1024*4)+(row_count*4))),
													cmem_out_buffer0,
													goodcolumns,
													(unsigned int*) (mb3_dma1_BASEADDR)
												);
			/*								
			} else {
				for (j = 0; j < goodcolumns; j++) {
					cmem_out_buffer1[j] = (ColorBuffer[(i * MCU_sx) + j]);
				}
				hw_dma_send_non_block_addr( 	(unsigned int*) (shared_pt_REMOTEADDR+((col_count*4)+(i*1024*4)+(row_count*4))),
													cmem_out_buffer1,
													goodcolumns,
	...
	else if((MCU_row < parts[2]) && (MCU_row >= parts[0]))
	{
		for (curcomp = 0; MCU_valid[curcomp] != -1; curcomp++) {
