...
extern int MCU2_BLOCK;
extern int mcu2_count;
extern unsigned int* cmem2_out_buffer;
int WordSize2 =  mb2_cmemout0_SIZE >> 3;
int ByteSize2 =  mb2_cmemout0_SIZE >> 1;
...
int process_MCU()
{
	...
	if((MCU_row < parts[2]) && (MCU_row >= parts[1]))
	{
		...
		// for (i = 0; i < goodcolumns; i++) {
			// for (j = 0; j < goodrows; j++) {
				// cmem_out_buffer[(j + (i<<9) + (mcu_count*MCU_sy)] = (ColorBuffer[(i * MCU_sx) + j]);
			// }
		// }


		mcu2_count++;
		x_offset = (mcu2_count*MCU_sx*MCU_sy);
		
		if( (x_offset >= x_size*MCU_sy) )
		{
			for (i = 0; i < goodrows; i++) {
			//y_offset = i+ (mcu_count*MCU_sy);
				mb2_offset = (( i + (MCU_row*MCU_sy)) *4*1024);
				
				for (j = 0; j < x_size; j++) {
					cmem2_out_buffer[j] = ColorBuffer[j + i*x_size ]; //0XFF000000;
				}
				hw_dma_send_non_block_addr( 	(unsigned int*) (shared_pt_REMOTEADDR+mb2_offset), //Baseaddr + (amount of MCU rows * bits in MCU y direction * 1024 bits in x dir * 4 bytes)
													cmem2_out_buffer,
													x_size,
													(unsigned int*) (mb2_dma0_BASEADDR)
												);
				MCU2_BLOCK++;
			}
			mcu2_count = 0;
	...
	else if((MCU_row < parts[1]) && (MCU_row >= parts[0]))
	{
		for (curcomp = 0; MCU_valid[curcomp] != -1; curcomp++) {
